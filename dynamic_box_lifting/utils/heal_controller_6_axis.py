...
class HealController6Axis:
    ...
    def _ft_callback(self, msg: WrenchStamped):
        ...
        # --- Slip-prevention logic updates desired lambda_star ---
        mu = 0.5
        base_normal = 7.0
        safety = 1.1

        # Check tangential load in x (external pushing down)
        if abs(msg.wrench.force.x) / (mu * base_normal) >= 1.8:
            # Increase desired squeeze force in x (normal direction)
            new_fn = abs(msg.wrench.force.x) * safety / mu
            self.lambda_star = np.array([-new_fn - base_normal, 0, 0, 0, 0, 0])
        else:
            # Default baseline desired force
            self.lambda_star = np.array([-base_normal, 0, 0, 0, 0, 0])
